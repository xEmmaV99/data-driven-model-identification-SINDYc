...
def optimize_simulation(path_to_data_files, nmbr_models=-1, loglwrbnd=None, loguprbnd=None):
    """
    ...
    parameter_search(np.logspace(loglwrbnd[0], loguprbnd[0], nmbr_models),
                     train_and_validation_data=[train, DATA['x_train'], DATA['u_train'], val, DATA['x_val'],
                                                DATA['u_val']],
                     method="SR3_L1", name="umptorque_sr3", plot_now=False, library=library)
    print("Lasso optimisation")
    parameter_search(np.logspace(loglwrbnd[1], loguprbnd[1], nmbr_models),
                     train_and_validation_data=[train, DATA['x_train'], DATA['u_train'], val, DATA['x_val'],
                                                DATA['u_val']],
                     method="lasso", name="umptorque_lasso", plot_now=False, library=library)

    return


def make_model(path_to_data_files, alpha, optimizer, nmbr_of_train=-1):
    """
    ...
    save_model(model, "UMP-torque_model", lib)
...
def simulate(model_name, path_to_test_file):
    ...
    UMP_test = TEST['UMP']
    T_test = TEST['T_em']
    test_values = np.hstack((T_test.reshape(-1, 1), UMP_test))
    x_test = TEST['x']
    u_test = TEST['u']
    ...
    xydata = [np.hstack((t, test_predicted[:, 0].reshape(-1, 1))), np.hstack((t, T_test.reshape(-1, 1))),
              np.hstack((t, Torque.reshape(-1, 1)))]
    xlab = r"$t$"
...
if __name__ == "__main__":
    ...
    simulate("UMP-torque_model-good-sr3", path_to_test_file)
