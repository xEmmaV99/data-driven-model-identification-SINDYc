...
try:
    from pysindy_tmp import FiniteDifference
except ImportError:
...
def prepare_data(
    ...
    if not test_data and number_of_trainfiles > dataset["i_st"].shape[2]:
        raise ValueError(
            "number_of_trainfiles is larger than the amount of simulations in the dataset"
        )
    if number_of_trainfiles == -1 or number_of_trainfiles == "all":
        number_of_trainfiles = len(V_range)
    random_idx = random.sample(
        range(len(V_range)), number_of_trainfiles
    )  # the simulations are shuffled
    ...
    if use_estimate_for_v:
        v_stator = reference_abc_to_dq0(
            v_abc_estimate_from_line(dataset["v_applied"])
        )
    else:
        path_to_simulation_data = os.path.join(
            os.path.dirname(path_to_data_file), "SIMULATION_DATA.pkl"
        )
        v_stator = reference_abc_to_dq0(
            v_abc_calculation(dataset, path_to_motor_info=path_to_simulation_data)
        )
    ...
    for simul in range(number_of_trainfiles):
        if simul == 0:  # initiliaze
            I = scipy.integrate.cumulative_trapezoid(
                i_st[:, :, simul], dataset["time"][:, 0, simul], axis=0, initial=0
            )
            V = scipy.integrate.cumulative_trapezoid(
                v_stator[:, :, simul], dataset["time"][:, 0, simul], axis=0, initial=0
            )
            continue
        I = np.dstack(
            (
    ...
    ###  x_data = np.hstack((i_st, I.reshape(i_st.shape), V.reshape(i_st.shape))) ## Merged model is yet to be implemented
    t_data = dataset["time"]
    if np.ndim(x_data) <= 2:
    ...
    DATA["x"] = x_data.transpose(0, 2, 1).reshape(
        x_data.shape[0] * x_data.shape[-1], x_data.shape[1]
    )
    DATA["u"] = u_data.transpose(0, 2, 1).reshape(
        u_data.shape[0] * u_data.shape[-1], u_data.shape[1]
    )
    DATA["xdot"] = xdots.transpose(0, 2, 1).reshape(
        xdots.shape[0] * xdots.shape[-1], xdots.shape[1]
    )
    DATA["T_em"] = (
        dataset["T_em"]
        .transpose(0, 2, 1)
        .reshape(dataset["T_em"].shape[0] * dataset["T_em"].shape[-1])
    )
    DATA["UMP"] = (
        dataset["F_em"]
        .transpose(0, 2, 1)
        .reshape(
            dataset["F_em"].shape[0] * dataset["F_em"].shape[-1],
...
def check_trapezoid_integration():
    ...
    path = os.path.join(
        os.getcwd(), "test-data", "07-29-default", "IMMEC_0ecc_5.0sec.npz"
    )
...
def reference_abc_to_dq0_CP(coord_array: np.array, gamma):
    ...
    if np.ndim(gamma) <= 2:
        ...
        coord_array = coord_array.reshape(
            (coord_array.shape[0], coord_array.shape[1], 1)
        )
...
def v_abc_calculation(data_logger: dict, path_to_motor_info: str):
    ...
    """
    # read motor data to get values for R, Nt, L_s
    ...
    # use finite differences to calculate dphi and di, 4rth order
    dphi = FiniteDifference(order=4, axis=0)._differentiate(
        data_logger["flux_st_yoke"], t
    )
    di = FiniteDifference(order=4, axis=0)._differentiate(data_logger["i_st"], t)

    ist = data_logger["i_st"]
    v_abc = (
        np.tensordot(R, ist.swapaxes(0, 1), axes=([1], [0]))
...
def v_abc_estimate_from_line(v_line: np.array):
    ...
    return (
        1 / 3 * np.tensordot(T, v_line.swapaxes(0, 1), axes=([1], [0])).swapaxes(0, 1)
    )
...
def read_V_from_data(path_to_data_file: str):
    ...
    with open(path_to_data_file, "rb") as file:
        ...
        # read the max value of the voltage from the data, and divide by sqrt(2) to get the rms value
    return V_values
...
def calculate_xdot(x: np.array, t: np.array):
    ...
    """
    if np.ndim(t) == 3:
        print("Assume all t_vec are equal for all simulations")
        # I see not why this would not be the case, but it is not guaranteed
        t_ = t[:, 0, 0].reshape(t.shape[0])
        return FiniteDifference(order=4, axis=0)._differentiate(x, t_)
        # t should have shape (N, )
    return FiniteDifference(order=4, axis=0)._differentiate(
        x, t.reshape(t.shape[0])
    )  # default order is two


if __name__ == "__main__":
    # Quick test
    path = os.path.join(
        os.getcwd(), "train-data", "07-29-default", "IMMEC_0ecc_5.0sec.npz"
    )
    path = os.path.join(
        os.getcwd(), "test-data", "07-31-leuven", "IMMEC_90ecc_5.0sec.npz"
    )
    data = prepare_data(
        path,
        test_data=True,
