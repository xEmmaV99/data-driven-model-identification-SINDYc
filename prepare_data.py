...
def prepare_data(path_to_data_files, V_test_data=None, Torque=False, UMP=False, path_to_test_file=None, t_end=1.0,
    ...
    for V_value in V_range:
        # load numpy file
        '''
        path = path_to_data_files + '\\IMMEC_history_' + str(V_value) + 'V_' + str(t_end) + 'sec.npz'
        dataset = np.load(path) # shoudl be a dictionary
        ''' #load pickle file
        ...
        if normalize_input:
            raise NotImplementedError('Normalization should not be used')
